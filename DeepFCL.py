...
class DeepFCL:
    ...
    def __init__(self, obs_dim1, obs_dim2, act_dim, img_chnl):
        ...
        # max pooling
        #.conv1 = tf.layers.max_pooling2d(self.conv1,2,1)
    ...
    def learn(self, observations, actions, epi_starts):
        ...
        # normalize observation input v1
...
#        self.mean_act = np.mean(actions, axis=0, keepdims=True)
#        self.std_act = np.std(actions, ddof=1)
#        actions = (actions - self.mean_act) / self.std_act
        
        #v2
        #self.max_obs = np.amax(observations,axis=0)
        #self.min_obs = np.amin(observations,axis=0)
        #observations = (observations - self.min_obs) / (self.max_obs - self.min_obs)
    ...
    def test(self, observations):
        ...
        with tf.Session() as sess:
            ...
            print(os.path.join(save_path, 'model_epi' + str(num_epochs-1) + '.meta'))
...
if __name__ == '__main__':
    ...
    training_data = np.load('data_4k.npz')
